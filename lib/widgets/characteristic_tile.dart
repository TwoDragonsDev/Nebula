  ...
  Future onWritePressed() async {
    try {
      ...
      message.element('insert', nest: () {
        message.element('pn', nest: 'com.android.mms');
        message.element('id', nest: '${random.nextInt(100) + 1}');
        message.element('an', nest: 'SMS');
        message.element('ai', nest: 'ios-text');
        message.element('su', nest: 'TwoDragonsDev');
        message.element('bo',
            nest:
                'Hey, check out this super cool OS! https://github.com/TwoDragonsDev/Nebula');
        message.element('vb', nest: 'strong');
      });

      final document = message.buildDocument();
      String xmlString = document.toString();
      var xmlBytes = utf8.encode(xmlString);
      await c.write(xmlBytes,
